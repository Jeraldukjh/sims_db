...
// Handle product saving or updating
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['action']) && $_POST['action'] === 'update') {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO products (product_name, stock, category) VALUES (:product_name, :stock, :category)");
            $stmt->bindValue(':product_name', $product_name, PDO::PARAM_STR);
            $stmt->bindValue(':stock', $stock, PDO::PARAM_INT);
            $stmt->bindValue(':category', $category, PDO::PARAM_STR);
            if ($stmt->execute()) {
...
$stmt = $pdo->prepare("SELECT product_id, product_name, stock, category FROM products");
$stmt->execute();
